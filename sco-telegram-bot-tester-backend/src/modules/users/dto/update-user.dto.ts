...
export class UpdateUserDto {
  ...
  @ApiProperty()
  @IsOptional()
  @IsString({ message: validationErrorMessages.USERS.NEW_PASSWORD.INVALID_VALUE })
  @MinLength(8, { message: validationErrorMessages.USERS.NEW_PASSWORD.MIN_LENGTH })
  @Matches(
    /((?=.*\d)|(?=.*\W+))(?![.\n])(?=.*[A-Z])(?=.*[a-z]).*$/,
    { message: validationErrorMessages.USERS.NEW_PASSWORD.MATCHES }
  )
  newPassword?: string;
